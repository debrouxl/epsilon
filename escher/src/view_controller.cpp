...
void ViewController::convertFloatToText(double value, char * buffer, const char * header, size_t headerLength, const char * trailer, int precision) {
  ...
  numberOfChar += Poincare::Complex<double>::convertFloatToText(value, buffer+numberOfChar, Poincare::PrintFloat::bufferSizeForFloatsWithPrecision(precision), precision);
