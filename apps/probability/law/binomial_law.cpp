...
T BinomialLaw::templatedEvaluateAtAbscissa(T x) const {
  ...
  T lResult = std::lgamma(parameter1+1) - std::lgamma(std::floor(x)+1) - std::lgamma((T)parameter1 - std::floor(x)+1)+
    std::floor(x)*std::log(parameter2) + (parameter1-std::floor(x))*std::log((T)1-parameter2);
