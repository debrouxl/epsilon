...
void FunctionBannerDelegate::reloadBannerViewForCursorOnFunction(CurveViewCursor * cursor, Function * function, char symbol) {
  ...
  numberOfChar += Complex<float>::convertFloatToText(cursor->y(), buffer+sizeof("0(x)=")-1, PrintFloat::bufferSizeForFloatsWithPrecision(Constant::MediumNumberOfSignificantDigits), Constant::MediumNumberOfSignificantDigits);
  strcpy(buffer+numberOfChar, space);
